    ...
    try:
        ...
        if not df.empty and len(df) > 5:
            ...
            if 'Open' in df.columns and 'Close' in df.columns:
                fig = go.Figure(data=[go.Candlestick(x=df.index, open=df['Open'], high=df['High'], low=df['Low'], close=df['Close'], increasing_line_color='#00ff41', decreasing_line_color='#eb4034')])
                fig.update_layout(xaxis_rangeslider_visible=False, plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font=dict(color='#e0e0e0'), margin=dict(l=10, r=10, t=30, b=10), height=350, title=dict(text=f"GRÁFICO DIÁRIO: {ticker}", x=0.5, font=dict(size=14, color='#00f3ff')))
                return fig
        return None
    except: return None
