                ...
                with c5:
                    ...
                    with ac1:
                        if rec_qty > 0:
                            ...
                            debug_keys = list(ai_plan.get('allocations', {}).keys())
                            # LIMIT DEBUG to first 3 keys to avoid UI explosion, or just show count
                            debug_str = str(debug_keys)[:50] + "..."
                            
                            st.markdown(f"<div style='padding-top:10px; text-align:right; margin-right:15px;'><span style='color:#555; font-size:11px; font-weight:bold'>MANTER</span><br><span style='font-size:8px; color:#444'>Keys: {debug_str}</span></div>", unsafe_allow_html=True)
                        else:
                            st.markdown("")
